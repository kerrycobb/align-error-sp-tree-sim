...
The affect dissapears when divergence time is larger than about $8N_e\mu$.
...
and on how the data are processed. The program Stacks2 first identifies alleles
then merges alleles together to create consensus loci.


\subsection{Practical Guidance}

Amount of error will be dependent on coverage and on bioinformatic processing.
If error is coming from bioinformatic processing then all singletons would 
be removed and error would be expected to be even worse than what is seen 
in our results. 

ipyrad uses a minimum depth of coverage per base to call variants with a default
value of 6 \citep{PyRADeatonAssemblyNovo2014}. 



If sequencing depth at a locus is low, then there is a possibility that one 
allele will be missed. This will also be dependant on the minimum number of 
reads that are required to identify an allele. The program Stacks2 has a default 
of 3 \citep{rochetteStacksAnalyticalMethods2019}. 


Assuring to see affect of bias limited to small region of parameter space and
only large at most severe error rates.
...
various steps in data collection process.
Also, real data were not generated under the model with no prior
misspecification.
Violations of the model might make methods more sensitive
to lower rates of error.
