...
Phylogeneticists inferring species trees from reduced-representation genomic 
data sets using full likelihood Bayesian methods are faced with two 
classes of methods when conducting their analyses.
These classes differ in the way they model the evolution of orthologous DNA 
...
% -----------------------------------------------------------------------------
% Reference other types of error? Some of which have been explored 
% for their efffect on phylogentics? 
...
\ecoevolity implements an unlinked-character model of sequence evolution and 
it is not uncommon for phylogeneticists to use only a single variable sites per locus
from their alignments when using methods implementing an unlinked-character model. 

We examined the effect of including the invariant sites in \ecoevolity.
...
\ecoevolity divergence-time estimates are only biased at
