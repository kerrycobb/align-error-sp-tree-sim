...
200 and PSRF less than 1.2 \citep{gelman1998} to indicate adequate convergence
...
types of errors occur during sample preparation, high-throughput sequencing,
...
\jrocomment{Are the following 4 sentences redundant (saying the same thing in
    two different ways)?}
To filter out sequencing errors, most pipelines for processing sequence data set 
a minimum coverage threshold for variants.
True variants are more likely to be filtered out when coverage is low. 
...
Such stringent filtering could introduce an even greater level of error than we
